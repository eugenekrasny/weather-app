...
function CityForecastView(props) {
  ...
  const [units, setUnits] = useState(
    localStorage.getItem('weatherAppUnits') || 'imperial',
  );
  ...
  return (
    <div className="city-forecast">
      <CityForecastHeader
        onBackClickHandler={navigateToCitySelector}
        cityName={weather?.requestedCity?.name || cityName}
      />
      <UnitsSwitch
  ...
  return (
    <h2 className="header">
      <button
        ...
        onClick={props.onBackClickHandler}
      >
...
function CurrentWeather(props) {
  ...
  if (slicedForecast) {
    const slicedItems = slicedForecast.map((wrappedForecast) => {
      return (
        <li key={wrappedForecast.caption}>
          <span className="slice-caption">{wrappedForecast.caption}</span>
          <span className="slice-temperature">
  ...
  return (
    <div className="current-weather">
      ...
      <h1 className="weather-description">
        {weather.temp[units]}
        <i className={'weather-icon wi ' + weather.conditionsIcon} />
      </h1>
...
function DailyForecast(props) {
  ...
  return <ul className="daily-forecast">{dailyItems}</ul>;
}
