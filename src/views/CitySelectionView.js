...
function CitySelectionView() {
    ...
    const weatherLoadedCallback = useCallback((error, result) => {
        if (error) {
            setIsLoading(false);
    ...
    const getCurrentLocation = useCallback(() => {
        ...
        var success = (position) => {
            DataLoader.loadWeatherDataByCoords(position.coords, weatherLoadedCallback);
        };
        var error = (err) => {
    ...
    if (errorMessage) {
        errorMessageComponent = <h4 className="error-message">Error: {errorMessage}</h4>
    }
    return (
        <div className="city-selection">
            <form className="city-search" onSubmit={onSearchClick} >
                <input className="city-search-field" type="text" placeholder="City" ref={(input) => { inputRef.current = input; }} maxLength="20" />
                <button type="submit" className="city-search-button"><i className="material-icons">&#xE8B6;</i></button>
            </form>
            <p className="or-caption">or</p>
            <p>use my <span className="use-current-position" onClick={getCurrentLocation}>current position</span></p>
